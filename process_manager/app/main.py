...
class Supervisor:
    ...
    async def handle_start(self, output_id: str, argv: list[str]) -> None:
        if output_id in self.processes:
            await self.handle_stop(output_id)
        mp = ManagedProcess(output_id=output_id, argv=argv)
        await mp.start()
        self.processes[output_id] = mp
        # TODO: spawn a task to read progress and publish health to Redis.

    async def handle_stop(self, output_id: str) -> None:
    ...
    async def run(self) -> None:
        ...
        print(f"[process-manager] up. Redis={redis_url}. Awaiting control commands…")
        # TODO(Phase 1): subscribe to Redis control channel `castcore:control` and
        #   dispatch {action: start|stop|restart, output_id, argv} to the handlers;
        #   reconcile against desired state in PostgreSQL on startup.
